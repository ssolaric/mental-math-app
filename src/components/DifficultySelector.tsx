...
export const DifficultySelector = ({ operation, onSelectDifficulty }: DifficultySelectorProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-green-100 to-emerald-100 p-8">
      <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">{t(`operations.${operation}`)}</h1>
      <p className="text-xl text-gray-600 mb-12">{t('difficulty.chooseLevel')}</p>
        ...
        {difficulties.map(({ level, color }) => (
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <p className="mt-2 text-lg opacity-90">{t(`difficulty.descriptions.${operation}.${level}`)}</p>
            </div>
          </button>
        ))}
      </div>
