  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-green-100 to-emerald-100 p-8">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-800 text-center md:text-left">{t('stats.yourProgress')}</h1>
        <button
      ...
      <div className="max-w-4xl mx-auto w-full space-y-8">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">{t('stats.overallStats')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex items-center gap-4">
              <Target className="text-blue-600" size={40} />
              <div>
                <p className="text-gray-600 text-sm">{t('stats.totalProblems')}</p>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">{t('stats.byOperation')}</h2>
            ...
            {(Object.keys(gameProgress.stats) as Array<keyof typeof gameProgress.stats>).map((operation) => {
              ...
              return (
                <div key={operation} className="border-b border-gray-200 pb-6 last:border-b-0 last:pb-0">
                  <h3 className="text-xl font-bold text-gray-800 mb-3">{t(`operations.${operation}`)}</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    <div>
                      <p className="text-gray-600 text-sm">{t('stats.attempts')}</p>
                    ...
                    <div>
                      <p className="text-gray-600 text-sm">{t('stats.bestScore')}</p>
